...
contract('OnlineMarketPlace', function (accounts) {
	...
	if (tx.logs[0].event) {
        ...
        assert.equal(mystoreId, storeId , 'Passed Product should be added to a specific store')
    ...
    it("should update price with provided storeId,productId and updatedPricetr", async() => {
        ...
        var updatedPrice=22
        var unitPrice
    const tx = await onlineMarketPlace.updatePrice(storeId,productId,updatedPrice,{from:accounts[2]})
    ...
    const tx = await onlineMarketPlace.removeProduct(storeId,productId,{from:accounts[2]})
